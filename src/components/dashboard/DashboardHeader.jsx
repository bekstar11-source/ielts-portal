        ...
        <nav className="flex items-center gap-4 md:gap-8 text-[15px] md:text-[16px] overflow-x-auto max-w-full px-2 scrollbar-hide">
          {menuItems.map((item) => (
            <button
              ...
              className={`font-medium transition-all relative nav-link whitespace-nowrap pb-1
                        ${activeTab === item.id
            ...
            >
              {item.label}
              {activeTab === item.id && (
                <span className="absolute bottom-0 left-0 w-full h-0.5 bg-vetra-blue rounded-full"></span>
              )}
            </button>
          ))}
        </nav>
            ...
            {isProfileOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-[#0F0F0F] border border-white/10 rounded-xl shadow-xl overflow-hidden py-1 z-50 animate-fade-in-up">
                ...
                <button
                  ...
                  className="w-full text-left px-4 py-2 text-sm text-white hover:bg-white/5 flex items-center gap-2 transition-colors border-b border-white/5"
                >
                  <Key className="w-4 h-4" /> {/* Reuse Key icon or add generic Settings icon if available, but for now generic is fine or no icon */}
                  Sozlamalar
                </button>
