...
import { Volume2, Volume1, VolumeX, List, Flag } from 'lucide-react';
...
const ReviewHeader = ({ 
    ...
    const isMockFull = resultData.type === 'mock_full';
    const overallValue = isMockFull
    ...
    return (
        <header className="bg-white text-gray-900 shrink-0 z-20 border-b border-gray-100 relative shadow-sm">
            {/* SINGLE ROW: IDENTITY + CONTROLS + SCORE */}
            <div className="h-16 flex justify-between items-center px-3 sm:px-6 gap-3">
                ...
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="flex flex-col min-w-0">
                        <h1 className="text-[14px] font-semibold text-gray-900 tracking-tight truncate max-w-[140px] sm:max-w-[280px]">
                            {testData.title}
                        </h1>
                        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                            ...
                            <span className="text-[9px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">
                                {isMockFull ? 'Full Mock' : 'Partial'}
                            </span>
                            {testData.tags && testData.tags.length > 0 && testData.tags.map((tag, idx) => (
                                <span key={idx} className="hidden sm:inline text-[9px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full bg-zinc-100 text-zinc-600">
                                    #{tag}
                                </span>
                            ))}
                        </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    {isPremium && testData.type?.toLowerCase() !== 'speaking' && testData.type?.toLowerCase() !== 'writing' && (
                        ...
                        >
                            <List size={16} />
                            <span className="text-[12px] font-semibold tracking-tight hidden lg:inline">
                                {t('testSolving.viewDetailedAnswers') || 'Answers List'}
                            </span>
                        </button>
                    )}

                    {/* Report an issue with this test — o'quvchi ko'rib turadigan, lekin
                        asosiy harakatlarni bosib ketmaydigan darajada ajratilgan */}
                    {onReportIssue && (
                        <button
                            onClick={onReportIssue}
                            title={t('testSolving.reportTestIssue') || 'Xato haqida xabar berish'}
                            className="h-9 px-3 rounded-lg border border-orange-200 bg-orange-50 text-orange-700 flex items-center gap-2 transition-all hover:bg-orange-100 hover:border-orange-300 active:scale-95"
                        >
                    ...
                    <div className="h-12 px-3 sm:px-4 rounded-xl bg-red-50/70 border border-red-100 flex flex-col items-center justify-center min-w-[64px] sm:min-w-[76px]">
            ...
            {/* AUDIO ROW — faqat listening uchun (aks holda ikkinchi header bo'sh turardi) */}
            {hasAudioPlayer && (
                <div className="h-14 flex items-center gap-3 px-4 sm:px-6 bg-gray-50/60 border-t border-gray-100">
