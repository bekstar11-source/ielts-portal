        ...
        <div className={`rounded-[24px] p-6 border transition-colors ${isDark ? 'bg-[#272727] border-white/5' : 'bg-white border-gray-200 shadow-sm'}`}>
            ...
            <div className="space-y-2">
                {users.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((user) => {
                    ...
                    return (
                        <div key={user.id} className={`flex flex-col md:flex-row items-start md:items-center justify-between p-2 bg-gray-50 dark:bg-[#303030] rounded-xl hover:bg-gray-100 dark:hover:bg-[#383838] transition border border-gray-200 dark:border-white/5 gap-3 group ${user.isBlocked ? 'opacity-50 grayscale' : ''}`}>
                            <div className="flex items-center gap-3 w-full md:w-auto cursor-pointer" onClick={() => onSelectUser(user)}>
                                ...
                                <div>
                                    <div className="flex items-center gap-2">
                                        <p className="text-xs font-bold text-gray-900 dark:text-gray-200 leading-tight hover:underline">{user.fullName || "Ismsiz"}</p>
                                        {user.isBlocked && <Lock className="w-3 h-3 text-red-500" />}
                                    </div>
                                    <p className="text-[10px] text-gray-500">{isOnline ? "Online" : timeText}</p>
                                    {isOnline && user.currentActivity && (
                                        <p className="text-[10px] text-blue-500 animate-pulse mt-0.5 flex items-center gap-1">
                            ...
                            <div className="flex items-center justify-between md:justify-end w-full md:w-auto gap-2">
                                <span className={`px-2 py-1 rounded-lg text-[9px] font-bold border ${userGroup ? 'bg-purple-100 border-purple-200 text-purple-600' : 'bg-blue-100 border-blue-200 text-blue-600'}`}>
