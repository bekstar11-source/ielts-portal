...
export default function DiffModal({ show, originalJson, currentJson, onClose, onConfirm, isDark }) {
    ...
    return (
        <div className="fixed inset-0 z-[205] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className={`w-full max-w-2xl max-h-[90dvh] rounded-2xl shadow-2xl border overflow-hidden ${isDark ? 'bg-[#1f1e1b] border-white/10' : 'bg-white border-gray-200'}`}>
                <div className="p-5">
                    ...
                    <div className={`rounded-xl border overflow-auto max-h-[55dvh] custom-scrollbar ${isDark ? 'bg-[#181715] border-white/5' : 'bg-gray-50 border-gray-200'}`}>
                        {diff.tooBig ? (
