        ...
        <div className="w-full overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody className="divide-y divide-zinc-100 dark:divide-white/5">
                    {tests.map(test => {
                        // Extract unique question types from both flat questions array and passages
                        ...
                        const qTypes = Array.from(new Set(allQuestions.map(q => q.questionType || q.type))).filter(Boolean);
                        const passageCount = test.passages?.length || (test.type === 'listening' ? 4 : (test.type === 'reading' ? 3 : 0));
                        const totalGroups = allQuestions.length;
                        return (
                                ...
                                <td className="py-4 px-4">
                                    <div className="flex flex-col gap-0.5">
                                        <span className={`text-xs font-bold ${isDark ? 'text-zinc-300' : 'text-zinc-700'}`}>
                                            {passageCount} {test.type === 'listening' ? 'Parts' : 'Passages'}
                                        </span>
