...
export const RecentAwards = ({ awards, isDark }) => (
    <div className={`rounded-[24px] border p-6 flex flex-col transition-colors ${isDark ? 'bg-[#1E1E1E] border-white/5' : 'bg-white border-gray-200 shadow-sm'}`}>
            ...
            {awards.length > 0 ? (
                awards.map((award, i) => (
                    <div key={i} className="flex gap-4 animate-in slide-in-from-right-2 duration-300">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${isDark ? 'bg-white/5 text-purple-400' : 'bg-purple-50 text-purple-600'}`}>
                            <Medal size={24} />
                        </div>
        ...
        <button className="w-full mt-6 py-3 rounded-xl border border-dashed border-zinc-300 dark:border-white/10 font-bold text-xs uppercase tracking-widest text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 hover:border-zinc-400 transition-all">
            View All History
        </button>
    </div>
);
