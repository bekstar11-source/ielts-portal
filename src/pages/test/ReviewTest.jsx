...
export default function ReviewTest() {
  ...
  useEffect(() => {
    if (testData?.writingTasks?.length > 0) {
      setActiveWritingTab(testData.writingTasks[0].id);
    }
  }, [testData]);
                 ...
                 {testData?.tags && testData.tags.length > 0 && (
                     <div className="flex items-center gap-1.5">
                         {testData.tags.map((tag, idx) => (
                             <span key={idx} className="text-[9px] font-medium uppercase tracking-wider px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-300 border border-zinc-700">
                                 #{tag}
                             </span>
                         ))}
                     </div>
             ...
             <div className="w-full h-full flex flex-col bg-gray-50 overflow-hidden">
                 ...
                 <div className="flex-1 flex flex-col overflow-hidden">
                     {testData.writingTasks ? (
                         <>
                             {/* Tabs */}
                             <div className="bg-white border-b px-6 py-2 flex gap-4 shadow-sm sticky top-0 z-10 justify-center shrink-0">
                                 {testData.writingTasks.map(task => (
                                     <button
                                         key={task.id}
                                         onClick={() => setActiveWritingTab(task.id)}
                                         className={`px-4 py-2 text-sm font-semibold rounded-lg transition ${
                                             activeWritingTab === task.id 
